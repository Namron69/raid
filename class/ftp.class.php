...
class Ftp
{
	...
	public function __call($name, $args)
	{
		...
		if (!function_exists($func)) {
			throw new Exception("Обращение к неопределенной метод Ftp::$name().");
		}
		...
		if ($func === 'ftp_connect' || $func === 'ftp_ssl_connect') {
			...
			throw new FtpException("Не подключен к FTP-серверу.");
	...
	public function mkDirRecursive($dir)
	{
		...
		while (!empty($parts)) {
			...
			try {
				...
				if (!$this->isDir($path)) {
					throw new FtpException("Невозможно создать каталог \"$path\".");
				}
			}
			$path .= '/';
		}
